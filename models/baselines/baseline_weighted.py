...
for epoch in range(epochs):
    ...
    for id_batch, (x_batch, y_batch) in enumerate(dataset):
        ...
        prev_hour = y_train[id_batch-1]
        curr_hour_hist = averages[int(x_batch[-1].item())][int(x_batch[1].item())]
        if int(x_batch[1].item()) > 0:
            prev_hour_hist = averages[int(x_batch[-1].item())][int(x_batch[1].item()) - 1]
        else:
            try:
                prev_hour_hist = averages[int(x_batch[-1].item())-1][-1]
            except:
                prev_hour_hist = averages[365][-1]
        delta = curr_hour_hist - prev_hour_hist
        frac = prev_hour / prev_hour_hist
        if prev_hour_hist == 0:
            frac = 1
        guess = prev_hour + (delta*frac)
