...
try:
    if resume:
        state = torch.load("saves/cyc_offset_saved_state")
...
run_num = 1
folder_path = f"results/model_results/offsets/cyc_offset_{offset}_results/run_"
...
if resume:
    ...
    if overwrite_save:
        torch.save(state, "saves/cyc_offset_saved_state")
    else:
        save_id = 1
        while os.path.exists("saves/cyc_offset_saved_state" + f"_{save_id}"):
            save_id += 1
        plt.savefig("saves/cyc_offset_saved_state" + f"_{save_id}")
