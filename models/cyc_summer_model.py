...
x_train = train_dataset.drop(columns = ['GHI', 'Minute', 'DayOfYear', 'Precipitation', 'Month', 'Hour', 'Year'])
y_train = train_dataset[['GHI']]
x_test = test_dataset.drop(columns = ['GHI', 'Minute', 'DayOfYear', 'Precipitation', 'Month', 'Hour', 'Year'])
y_test = test_dataset[['GHI']]

# get column titles for ColumnTransformer, excluding cyclical features
x_columns = ['Day', 'Temperature', 'Dew Point', 'Relative Humidity', 'Pressure', 'Wind Speed']
...
try:
    if resume:
        state = torch.load("saves/cyc_saved_state")
...
run_num = 1
folder_path = "results/model_results/cyc_summer_results/run_"
...
if resume:
    ...
    if overwrite_save:
        torch.save(state, "saves/cyc_saved_state")
    else:
        save_id = 1
        while os.path.exists("saves/cyc_saved_state" + f"_{save_id}"):
            save_id += 1
        plt.savefig("saves/cyc_saved_state" + f"_{save_id}")
