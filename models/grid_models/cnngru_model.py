...
class CNNGRU(nn.Module):
    def __init__(self, in_channels, horizon, d_model=128, num_layers=1):
        super().__init__()
        ...
        self.fc = nn.Linear(2*d_model, horizon)
...
model = CNNGRU(in_channels=input_dim, horizon=horizon).to(device)
...
path = f"results/grid_results/{target.lower()}/cnngru_{seq_len}"
...
plt.title("CNN+GRU GHI Pred vs Actual")
plt.legend()
plt.ylabel("GHI")
plt.xlabel("Hour")
...
if energy_metrics:
    ...
    plt.title("CNN+GRU Energy Pred vs Actual")
    plt.legend()
