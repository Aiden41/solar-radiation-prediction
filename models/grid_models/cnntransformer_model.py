...
class CNNTransformer(nn.Module):
    def __init__(self, in_channels, horizon, d_model=128, nhead=4, num_layers=2, dropout=0.1):
        super().__init__()
...
model = CNNTransformer(in_channels=input_dim, horizon=horizon, dropout=0.1).to(device)
...
path = f"results/grid_results/{target.lower()}/cnntransformer_{seq_len}"
...
plt.title("CNN+Transformer GHI Pred vs Actual")
plt.legend()
plt.ylabel("GHI")
plt.xlabel("Hour")
...
if energy_metrics:
    ...
    plt.title("CNN+Transformer Energy Pred vs Actual")
    plt.legend()
