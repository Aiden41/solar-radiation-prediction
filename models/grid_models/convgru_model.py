...
class ConvGRUCell(nn.Module):
    def __init__(self, input_dim, hidden_dim, kernel_size):
        ...
        self.layer_norm = nn.LayerNorm([3*hidden_dim, grid_rows, grid_rows])
...
path = f"results/grid_results/{target.lower()}/convgru_{seq_len}"
...
plt.title(f"ConvGRU ({seq_len} Rows) GHI Pred vs Actual")
plt.legend()
plt.ylabel("GHI")
plt.xlabel("Hour")
...
if energy_metrics:
    ...
    plt.title(f"ConvGRU ({seq_len} Rows) Energy Pred vs Actual")
    plt.legend()
