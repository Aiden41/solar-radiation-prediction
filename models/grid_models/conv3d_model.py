...
class TemporalBlock(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.conv7 = nn.Conv3d(in_channels, out_channels, kernel_size=(7,3,3), padding=(3,1,1))
...
class Conv3D(nn.Module):
    def __init__(self, in_channels):
        ...
        self.block1 = TemporalBlock(in_channels, 32)
        self.block2 = TemporalBlock(32, 64)
        self.block3 = TemporalBlock(64, 128)
    ...
    def forward(self, x):
        ...
        h = self.gap(h).view(B, -1)
        return self.fc(h)

device = 'cuda' if torch.cuda.is_available() else 'cpu'

train_ds = SolarDataset(x_train, y_train, seq_len)
valid_ds = SolarDataset(x_valid, y_valid, seq_len)
test_ds = SolarDataset(x_test, y_test, seq_len)

train_loader = DataLoader(train_ds, batch_size=batch_size, shuffle=True)
valid_loader = DataLoader(valid_ds, batch_size=batch_size, shuffle=False)

input_dim = train_ds[0][0].shape[1]
model = Conv3D(in_channels=input_dim).to(device)
...
path = f"results/grid_results/{target.lower()}/conv3d_{seq_len}"
...
plt.title("Conv3D GHI Pred vs Actual")
plt.legend()
plt.ylabel("GHI")
plt.xlabel("Hour")
...
if energy_metrics:
    ...
    plt.title("Conv3D Energy Pred vs Actual")
    plt.legend()
