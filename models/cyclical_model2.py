...
# mask = ((train_dataset['Month'] == 2) | (train_dataset['Month'] == 3) | (train_dataset['Month'] == 4) | (train_dataset['Month'] == 5) | (train_dataset['Month'] == 6) | (train_dataset['Month'] == 7) | (train_dataset['Month'] == 8) | (train_dataset['Month'] == 9) | (train_dataset['Month'] == 10))
# train_dataset = train_dataset[mask].copy()
# mask = ((test_dataset['Month'] == 2) | (test_dataset['Month'] == 3) | (test_dataset['Month'] == 4) | (test_dataset['Month'] == 5) | (test_dataset['Month'] == 6) | (test_dataset['Month'] == 7) | (test_dataset['Month'] == 8) | (test_dataset['Month'] == 9) | (test_dataset['Month'] == 10))
# test_dataset = test_dataset[mask].copy()

# mask = ((train_dataset['Month'] == 1) | (train_dataset['Month'] == 11) | (train_dataset['Month'] == 12))
# train_dataset = train_dataset[mask].copy()
# mask = ((test_dataset['Month'] == 1) | (test_dataset['Month'] == 11) | (test_dataset['Month'] == 12))
# test_dataset = test_dataset[mask].copy()

# drop unused columns and get values out of dataframe
x_train = train_dataset.drop(columns = ['GHI', 'Minute', 'DayOfYear', 'Month', 'Hour', 'Year'])
y_train = train_dataset[['GHI']]
x_test = test_dataset.drop(columns = ['GHI', 'Minute', 'DayOfYear', 'Month', 'Hour', 'Year'])
...
validloader = DataLoader(validation_dataset, batch_size=int(batch_size), shuffle=False)
...
try:
    if resume:
        state = torch.load("saves/cyc2_saved_state")
...
run_num = 1
folder_path = "results/model_results/cyc_results2/run_"
...
def compare_outputs(sample_list, expected_list, months, epoch_num):
    ...
    for index, month in enumerate(months):
        ...
        loss = criterion(predicted, expected_list[index]).cpu()
...
if resume:
    ...
    if overwrite_save:
        torch.save(state, "saves/cyc2_saved_state")
    else:
        save_id = 1
        while os.path.exists("saves/cyc2_saved_state" + f"_{save_id}"):
            save_id += 1
        plt.savefig("saves/cyc2_saved_state" + f"_{save_id}")
