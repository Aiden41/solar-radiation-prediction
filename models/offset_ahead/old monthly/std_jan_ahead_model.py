...
try:
    if resume:
        state = torch.load("saves/jan_ahead_state")
...
run_num = 1
folder_path = f"results/model_results/ahead/jan_ahead_{offset}/run_"
...
if resume:
    ...
    if overwrite_save:
        torch.save(state, "saves/jan_ahead_state")
    else:
        save_id = 1
        while os.path.exists("saves/jan_ahead_state" + f"_{save_id}"):
            save_id += 1
        plt.savefig("saves/jan_ahead_state" + f"_{save_id}")
