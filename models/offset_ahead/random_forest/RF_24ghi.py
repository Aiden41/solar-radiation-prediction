...
for index, row in enumerate(x_train):
    ...
    new_row = np.concatenate([t, t_prev_sr]).flatten()
...
for index, row in enumerate(x_valid):
    ...
    t_prev_sr = y_valid_oldGHI[index-24:index].flatten()
    new_row = np.concatenate([t, t_prev_sr]).flatten()
    #new_row = np.append(t, t_prev_sr)
...
jul_test = x_test[4344:4416]
jul_actual = y_test[4344:4416]
...
run_num = 1
folder_path = f"results/model_results/ahead/RF_{offset}/run_"
...
np.save('rf_july', july_pred)
# jun_actual = np.array(jun_actual)
# np.save('actual_jun', jun_actual)
