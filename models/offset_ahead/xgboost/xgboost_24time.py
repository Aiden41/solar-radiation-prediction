...
for index, row in enumerate(x_train):
    ...
    t_prev_sr = y_train[index-24:index].flatten()
...
for index, row in enumerate(x_valid):
    if index < 24:
        continue
    t = row
    t_prev_day = x_valid[index-24:index].flatten()
    t_prev_sr = y_valid[index-24:index].flatten()
    new_row = np.concatenate([t, t_prev_day, t_prev_sr]).flatten()
    #new_row = np.append(t, t_prev_sr)
...
for index, row in enumerate(x_test):
    ...
    t_prev_sr = y_test[index-24:index].flatten()
    new_row = np.concatenate([t, t_prev_day, t_prev_sr]).flatten()
    #new_row = np.append(t, t_prev_sr)
...
# march_actual = y_test[1392:1464]
# apr_actual = y_test[2136:2208]
# jun_actual = y_test[4320:5040]
...
#start1 = timer() * 1000000 # microseconds
...
# apr_pred = model.predict(apr_test)
july_pred = model.predict(july_test)
# end2 = timer() * 1000000
# testing_time_taken = end2 - start2
# start3 = timer() * 1000000
hour_pred = model.predict(x_test[0].reshape(1,-1))
# end3 = timer() * 1000000
...
# march_pred = np.array(march_pred)
# np.save('xgboost_march', march_pred)
# apr_pred = np.array(apr_pred)
# np.save('xgboost_apr', apr_pred)
july_pred = np.array(july_pred)
np.save('xgboost_july', july_pred)
# march_actual = np.array(march_actual)
# np.save('actual_march', march_actual)
# apr_actual = np.array(apr_actual)
# np.save('actual_apr', apr_actual)
# july_actual = np.array(july_actual)
# np.save('actual_july', july_actual)

best_month_losses = []
best_month_nrmse = []
for index, month in enumerate(x_months):
    month_pred = model.predict(month)
    loss = np.sqrt(mean_squared_error(y_months[index], month_pred))
    best_month_losses.append(loss)
    avg_ghi = np.mean(y_months[index])
    best_month_nrmse.append(loss/avg_ghi)
...
print("Best Iteration: " + str(model.best_iteration))
...
with open(f"{folder_path}/results.txt", 'w') as file:
    ...
    file.write("Best Iteration: " + str(model.best_iteration) + "\n")
