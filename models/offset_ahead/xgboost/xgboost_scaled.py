...
x_train['oldGHI'] = y_scaler.transform(train_oldGHI)
x_valid['oldGHI'] = y_scaler.transform(valid_oldGHI)
x_test['oldGHI'] = y_scaler.transform(test_oldGHI)
...
dec_actual = y_test[7991:8735]

model = XGBRegressor(n_estimators=500, eval_metric='rmse', early_stopping_rounds=100, eta=0.1)
...
plt.plot(x_axis, results['validation_0']['rmse'], linestyle = '--', label='Training Loss', color = 'darkslateblue')
plt.plot(x_axis, results['validation_1']['rmse'], label='Validation Loss', color='darkorange')
...
dec_pred = model.predict(dec_test)

np.save('xgb_dec.npy', dec_pred)
np.save('dec_actual.npy', dec_actual)

train_pred = inverse_y_scaler.inverse_transform(train_pred.reshape(-1, 1))
valid_pred = inverse_y_scaler.inverse_transform(valid_pred.reshape(-1, 1))
test_pred = inverse_y_scaler.inverse_transform(test_pred.reshape(-1, 1))

unscaled_y_train = inverse_y_scaler.inverse_transform(y_train.reshape(-1, 1))
unscaled_y_valid = inverse_y_scaler.inverse_transform(y_valid.reshape(-1, 1))
unscaled_y_test = inverse_y_scaler.inverse_transform(y_test.reshape(-1, 1))

train_loss = np.sqrt(mean_squared_error(unscaled_y_train, train_pred))
valid_loss = np.sqrt(mean_squared_error(unscaled_y_valid, valid_pred))
test_loss = np.sqrt(mean_squared_error(unscaled_y_test, test_pred))
