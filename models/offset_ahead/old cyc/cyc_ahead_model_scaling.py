...
try:
    if resume:
        state = torch.load("saves/cyc_ahead_saved_state")
...
run_num = 1
folder_path = f"results/model_results/ahead/cyc_ahead_{offset}_results/run_"
...
for epoch in range(epochs):
    ...
    with torch.no_grad():
        ...
        for id_batch, (x_batch, y_batch) in enumerate(validloader):
            ...
            y_batch = y_batch.cpu().detach().numpy().reshape(-1,1)
            y_batch = y_scaler.inverse_transform(y_batch)
            y_batch = y_batch.reshape(1,-1)[0]
...
if resume:
    ...
    if overwrite_save:
        torch.save(state, "saves/cyc_ahead_saved_state")
    else:
        save_id = 1
        while os.path.exists("saves/cyc_ahead_saved_state" + f"_{save_id}"):
            save_id += 1
        plt.savefig("saves/cyc_ahead_saved_state" + f"_{save_id}")
