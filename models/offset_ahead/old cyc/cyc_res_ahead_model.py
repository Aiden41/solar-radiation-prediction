...
x_train = train_dataset.drop(columns = ['GHI', 'Minute', 'DayOfYear', 'Month', 'Hour', 'Year', 'DNI', 'DHI', 'Solar Zenith Angle'])
y_train = train_dataset[['GHI']]
x_test = test_dataset.drop(columns = ['GHI', 'Minute', 'DayOfYear', 'Month', 'Hour', 'Year', 'DNI', 'DHI', 'Solar Zenith Angle'])
y_test = test_dataset[['GHI']]
y_mean = y_test.mean()['GHI']

# get column titles for ColumnTransformer, excluding cyclical features
x_columns = ['Day', 'Temperature', 'Dew Point', 'Relative Humidity', 'Pressure', 'Wind Speed', 'Precipitation']
...
class SkipConnection(nn.Module):
    def __init__(self):
        ...
        self.fc1 = nn.Linear(27, 200)
        self.fc2 = nn.Linear(200, 200)
        self.fc3 = nn.Linear(200, 1)
    
    def forward(self, x):
        out1 = nn.functional.relu(self.fc1(x))
        out2 = nn.functional.relu(self.fc2(out1))
        out = self.fc3(out1 + out2)  # Skip connection
        return out
...
try:
    if resume:
        state = torch.load("saves/cyc_res_ahead_saved_state")
...
run_num = 1
folder_path = f"results/model_results/ahead/cyc_res_ahead_{offset}_results/run_"
...
if weight_graphs:
    ...
    plt.ylabel("Weight")
    plt.xlabel("Node")
    plt.title("Cyclical Residual Model Initial Weight Distribution")
    ...
    plt.ylabel("Weight")
    plt.xlabel("Node")
    plt.title("Cyclical Residual Model Initial Squared Weight Distribution")
    plt.savefig(f"{weight_path}/cyc_res_init_squared_weight_dist.pdf")
    plt.show(block=show_graph)
    plt.close()
...
def compare_outputs(sample_list, expected_list, months, epoch_num):
    ...
    for index, month in enumerate(months):
        ...
        plt.title(f"Cyclical Residual Model Ouputs for First 72 Hours of {month} at Epoch {epoch_num}\nLoss: {loss} RMSE")
...
if weight_graphs:
    ...
    plt.ylabel("Weight")
    plt.xlabel("Node")
    plt.title("Cyclical Residual Model End Weight Distribution")
    plt.savefig(f"{weight_path}/cyc_res_end_weight_dist.pdf")
    plt.show(block=show_graph)
    plt.close()
    ...
    plt.ylabel("Weight")
    plt.xlabel("Node")
    plt.title("Cyclical Residual Model End Squared Weight Distribution")
    plt.savefig(f"{weight_path}/cyc_res_end_squared_weight_dist.pdf")
    plt.show(block=show_graph)
    plt.close()
...
if resume:
    ...
    if overwrite_save:
        torch.save(state, "saves/cyc_res_ahead_saved_state")
    else:
        save_id = 1
        while os.path.exists("saves/cyc_res_ahead_saved_state" + f"_{save_id}"):
            save_id += 1
        plt.savefig("saves/cyc_res_ahead_saved_state" + f"_{save_id}")
...
if loss_graphs:
    ...
    plt.xlabel("Epoch")
    plt.legend(loc="upper right")
    plt.title(f"Cyclical Residual Model Losses w/ {learning_rate} Learning Rate")
    fig_id = 1
