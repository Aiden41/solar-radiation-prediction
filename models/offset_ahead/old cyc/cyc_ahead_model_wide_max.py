...
for week in range(53):
    mask = (train_dataset['DayOfYear'] >= start) & (train_dataset['DayOfYear'] <= end)
    week_of_years = train_dataset[mask].copy()
    max_of_hour = []
    for hour in range(24):
        max_of_hour.append(max(week_of_years.iloc[hour::24, :]['GHI']))
    start+=7
    end+=7
    train_max_of_week.append(max_of_hour)
    # if end >= 365:
    #     mask = (train_dataset['DayOfYear'] == 365)
    #     week_of_years = train_dataset[mask].copy()
    #     max_of_hour = []
    #     for hour in range(24):
    #         max_of_hour.append(max(week_of_years.iloc[hour::24, :]['GHI']))
    #     train_max_of_week.append(max_of_hour)
#print(train_max_of_week)
#print(len(train_max_of_week))
#print(len(train_max_of_week[0]))

start = 1
end = 7
test_max_of_week = []
for week in range(53):
    mask = (test_dataset['DayOfYear'] >= start) & (test_dataset['DayOfYear'] <= end)
    week_of_years = test_dataset[mask].copy()
    max_of_hour = []
    for hour in range(24):
        max_of_hour.append(max(week_of_years.iloc[hour::24, :]['GHI']))
    start+=7
    end+=7
    test_max_of_week.append(max_of_hour)
    # if end >= 365:
    #     mask = (test_dataset['DayOfYear'] == 365)
    #     week_of_years = test_dataset[mask].copy()
    #     max_of_hour = []
    #     for hour in range(24):
    #         max_of_hour.append(max(week_of_years.iloc[hour::24, :]['GHI']))
...
#print(test_max_of_week)
#print(len(test_max_of_week))
#print(len(test_max_of_week[0]))
#input()

train_max_of_week_final = []
for _ in range(7):
    week_num = 1
    for x in train_max_of_week:
        if week_num%53 == 0:
            for item in x:
                train_max_of_week_final.append(item)
        else:
            for _ in range(7):
                for item in x:
                    train_max_of_week_final.append(item)
...
test_max_of_week_final = []
week_num = 1
for x in test_max_of_week:
    if week_num%53 == 0:
        for item in x:
            test_max_of_week_final.append(item)
    else:
        for _ in range(7):
            for item in x:
                test_max_of_week_final.append(item)
...
# print(len(train_dataset['GHI']))
# print(len(test_max_of_week_final))
# input()

train_dataset['GHI'] = train_dataset['GHI']/train_max_of_week_final
test_dataset['GHI'] = test_dataset['GHI']/test_max_of_week_final
train_dataset['GHI'] = train_dataset['GHI'].fillna(0)
test_dataset['GHI'] = test_dataset['GHI'].fillna(0)
...
x_columns_test = ['Temperature', 'Dew Point', 'Relative Humidity', 'Pressure', 'Wind Speed', 'Precipitation', 'Cloud Type', 'Surface Albedo', 'DayOfYear_Sin', 'DayOfYear_Cos', 'Sin_Hour', 'Cos_Hour', 'Sin_Month', 'Cos_Month', 'Temperature_prev', 'Dew Point_prev', 'Relative Humidity_prev', 'Pressure_prev', 'Wind Speed_prev', 'Precipitation_prev', 'Cloud Type_prev', 'Surface Albedo_prev', 'DayOfYear_Sin_prev', 'DayOfYear_Cos_prev', 'Sin_Hour_prev', 'Cos_Hour_prev', 'Sin_Month_prev', 'Cos_Month_prev', 'GHI_prev']
...
try:
    if resume:
        state = torch.load("saves/cyc_ahead_wide_max_saved_state_1")
...
run_num = 1
folder_path = f"results/model_results/ahead/cyc_ahead_wide_max_{offset}_results/run_"
...
if resume:
    ...
    if overwrite_save:
        torch.save(state, "saves/cyc_ahead_wide_max_saved_state")
    else:
        save_id = 1
        while os.path.exists("saves/cyc_ahead_wide_max_saved_state" + f"_{save_id}"):
            save_id += 1
        torch.save(state, "saves/cyc_ahead_wide_max_saved_state" + f"_{save_id}")
