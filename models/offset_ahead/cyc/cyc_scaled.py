...
july_test = x_test[4344:4416]
july_actual = y_test[4344:4416]
july_actual = np.array(july_actual)
np.save('july_actual.npy', july_actual)
...
run_num = 1
folder_path = f"results/model_results/ahead/cyc_ahead_wide_{offset}_results/run_"
...
for epoch in range(epochs):
    ...
    with torch.no_grad():
        ...
        if curr_loss < min_loss:
            print(model[0].weight[0][23].item())
            ...
            # unscaled_loss = inverse_y_scaler.inverse_transform(np.array(test_loss).reshape(-1,1))
            # print(unscaled_loss)
            prediction = torch.from_numpy(inverse_y_scaler.inverse_transform(y_test_pred.cpu().reshape(-1,1)))
            actual = torch.from_numpy(inverse_y_scaler.inverse_transform(y_test.cpu().reshape(-1,1)))
            unscaled_best_loss = np.sqrt(criterion(prediction, actual).detach().numpy())
            min_loss = curr_loss
            ...
            for id_batch, (x_batch, y_batch) in enumerate(monthloader):
                ...
                loss = np.sqrt(loss.detach().numpy())
                # print(loss)
                # prediction = torch.from_numpy(inverse_y_scaler.inverse_transform(y_pred.cpu().reshape(-1,1)))
                # actual = torch.from_numpy(inverse_y_scaler.inverse_transform(y_batch.cpu().reshape(-1,1)))
                # print(np.sqrt(criterion(prediction, actual).detach().numpy()))
                month_losses.append(loss)
...
print(unscaled_best_loss)
print(unscaled_best_loss/test_y_mean)
train_loss = losses[best_epoch-1]
# march_pred = np.array(march_pred.cpu())
# np.save('cyc_march', march_pred)
# apr_pred = np.array(apr_pred.cpu())
# np.save('cyc_apr.npy', apr_pred)
july_pred = np.array(july_pred.cpu())
np.save('cyc_july.npy', july_pred)
