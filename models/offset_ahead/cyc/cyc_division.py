...
for day in range(1,366):
    ...
    for hour in range(0,24):
        ...
        hours_in_day.append(np.mean(y_train[mask]))
    averages[day] = hours_in_day

# drop unused columns and get values out of dataframe
x_train = train_dataset.drop(columns = ['GHI', 'Minute', 'Month', 'Year', 'DNI', 'DHI', 'Solar Zenith Angle', 'Day'])

x_valid = valid_dataset.drop(columns = ['GHI', 'Minute',  'Month', 'Year', 'DNI', 'DHI', 'Solar Zenith Angle', 'Day'])
y_valid = valid_dataset[['GHI']]
y_valid_oldGHI = valid_dataset[['oldGHI']]

x_test = test_dataset.drop(columns = ['GHI', 'Minute',  'Month', 'Year', 'DNI', 'DHI', 'Solar Zenith Angle', 'Day'])
...
for index, row in enumerate(x_train):
    ...
    day = int(row[18])
    hour = int(row[8])
    #new_row = np.append(new_row, averages[day][hour+1])
    if averages[day][hour] != 0:
        new_row = np.append(t, np.log(averages[day][hour]))
        new_y_train.append(np.log(t / averages[day][hour]))
    else:
        new_row = np.append(t, averages[day][hour])
        new_y_train.append(t)
...
for index, row in enumerate(x_valid):
    ...
    if averages[day][hour] != 0:
        new_row = np.append(t, np.log(averages[day][hour]))
        new_y_valid.append(np.log(t / averages[day][hour]))
    else:
        new_row = np.append(t, averages[day][hour])
        new_y_valid.append(t)
...
for index, row in enumerate(x_test):
    ...
    if averages[day][hour] != 0:
        new_row = np.append(t, np.log(averages[day][hour]))
        new_y_test.append(np.log(t / averages[day][hour]))
    else:
        new_row = np.append(t, averages[day][hour])
        new_y_test.append(t)
...
# x_train = np.delete(x_train, [8,18], 1)
# x_valid = np.delete(x_valid, [8,18], 1)
# x_test = np.delete(x_test, [8,18], 1)
...
# july_actual = torch.FloatTensor(july_actual).to(device)
...
validloader = DataLoader(validation_dataset, batch_size=8760, shuffle=False)
month_dataset = TensorDataset(x_test, y_test)
monthloader = DataLoader(month_dataset, batch_size=730, shuffle=False)
...
for epoch in range(epochs):
    ...
    with torch.no_grad():
        ...
        if curr_loss < min_loss:
            ...
            test_loss = criterion(y_test_pred, y_test).cpu()
...
# july_actual = np.array(july_actual.cpu())
