...
for epoch in range(epochs):
    ...
    for id_batch, (x_batch, y_batch) in enumerate(dataloader):
        ...
        loss = criterion(y_pred.squeeze()+x_batch[:,23], y_batch).cpu()
    ...
    with torch.no_grad():
        ...
        if curr_loss < min_loss:
            ...
            dec_pred = model(dec_test)
            test_loss = criterion(y_test_pred.squeeze()+x_test[:,23], y_test).cpu()
...
dec_pred = np.array(dec_pred.cpu())
np.save('cyc_dec.npy', dec_pred)
