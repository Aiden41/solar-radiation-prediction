...
# save results
with open(f"results/point_results/xgboost_{target.lower()}_lag_{max(lags)+1}.txt", 'w') as file:
...
plt.ylabel("GHI")
plt.xlabel("Hour")
plt.savefig(f"results/point_results/xgboost_{target.lower()}_lag_{max(lags)+1}.pdf")
...
if energy_metrics:
    ...
    plt.title(f"XGBoost ({max(lags)+1} Rows) Energy Pred vs Actual")
    plt.legend()
    plt.ylabel("Energy (Wh/m\u00b2)")
    plt.xlabel("Hour")
    plt.savefig(f"results/point_results/xgboost_{target.lower()}_lag_{max(lags)+1}_energy.pdf")
    plt.show(block=False)