...
model = GRU(input_dim=x_train.shape[1], hidden_dim=256, num_layers=2, dropout=0.1).to(device)
...
# save results
with open(f"results/point_results/gru_{seq_len}_{target.lower()}.txt", 'w') as file:
...
plt.title(f"GRU ({seq_len} Rows) GHI Pred vs Actual")
plt.legend()
plt.ylabel("GHI")
plt.xlabel("Hour")
plt.savefig(f"results/point_results/gru_{seq_len}_{target.lower()}.pdf")
...
if energy_metrics:
    ...
    plt.title(f"GRU ({seq_len} Rows) Energy Pred vs Actual")
    plt.legend()
    plt.ylabel("Energy (Wh/m\u00b2)")
    plt.xlabel("Hour")
    plt.savefig(f"results/point_results/gru_{seq_len}_{target.lower()}_energy.pdf")
    plt.show(block=False)