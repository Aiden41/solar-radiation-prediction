...
# create day of year and cyclical hour columns 
...
# save hour to cyclical graph and values
if cyclical_day_results:
    with open("results/other/cyclical_day/cyclical_day.txt", 'w') as file:
        file.write(str(train_dataset['Hour'][0:24]))
        file.write("\n\n")
        file.write(str(train_dataset['Sin_Hour'][0:24]))
        file.write("\n\n")
        file.write(str(train_dataset['Cos_Hour'][0:24]))
    plt.plot(train_dataset['Sin_Hour'][0:24], train_dataset['Cos_Hour'][0:24])
    plt.ylabel("Sin Hour")
    plt.xlabel("Cos Hour")
    plt.savefig("results/other/cyclical_day/cyclical_day.pdf")
    plt.show(block=show_graph)
    plt.close()

# drop unused columns and get values out of dataframe
x_train = train_dataset.drop(columns = ['GHI', 'Minute', 'Hour'])
y_train = train_dataset[['GHI']]
x_test = test_dataset.drop(columns = ['GHI', 'Minute', 'Hour'])
y_test = test_dataset[['GHI']]

# get column titles for ColumnTransformer, excluding cyclical features
x_columns = ['Year', 'Day', 'Month', 'Temperature', 'Dew Point', 'Relative Humidity', 'Pressure', 'Wind Speed', 'Precipitation', 'DayOfYear']
...
try:
    if resume:
        state = torch.load("saves/hour_saved_state")
...
if resume:
    ...
    if overwrite_save:
        torch.save(state, "saves/hour_saved_state")
    else:
        save_id = 1
        while os.path.exists("saves/hour_saved_state" + f"_{save_id}.pdf"):
            save_id += 1
        plt.savefig("saves/hour_saved_state" + f"_{save_id}.pdf")
...
plt.title(f"Cyclical Hour Model Losses w/ {learning_rate} Learning Rate")
fig_id = 1
figure_name = f"results/model_results/hour_lr{learning_rate}_epoch{epoch+current_epoch}"
