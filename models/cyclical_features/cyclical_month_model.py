...
# create day of year and cyclical month columns 
...
x_train = train_dataset.drop(columns = ['GHI', 'Minute', 'Month'])
y_train = train_dataset[['GHI']]
x_test = test_dataset.drop(columns = ['GHI', 'Minute', 'Month'])
y_test = test_dataset[['GHI']]

# get column titles for ColumnTransformer, excluding cyclical features
x_columns = ['Year', 'Day', 'Hour', 'Temperature', 'Dew Point', 'Relative Humidity', 'Pressure', 'Wind Speed', 'Precipitation', 'DayOfYear']
...
try:
    if resume:
        state = torch.load("saves/month_saved_state")
...
if resume:
    ...
    if overwrite_save:
        torch.save(state, "saves/month_saved_state")
    else:
        save_id = 1
        while os.path.exists("saves/month_saved_state" + f"_{save_id}.pdf"):
            save_id += 1
        plt.savefig("saves/month_saved_state" + f"_{save_id}.pdf")
...
plt.title(f"Cyclical Month Losses w/ {learning_rate} Learning Rate")
fig_id = 1
figure_name = f"results/model_results/month_lr{learning_rate}_epoch{epoch+current_epoch}"
